...
function generateRandomRecipes(count = 10) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const ingredients = Array.from({ length: faker.number.int({ min: 2, max: 6 }) }, () => ({
      name: faker.commerce.productMaterial(),
      grams: faker.number.int({ min: 50, max: 500 })
    }));

    const instructions = Array.from({ length: faker.number.int({ min: 3, max: 6 }) }, () => faker.lorem.sentence());
